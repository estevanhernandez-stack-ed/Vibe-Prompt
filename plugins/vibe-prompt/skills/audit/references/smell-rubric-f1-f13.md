...
4b. **Absent-system-instruction sub-case (v0.7).** If the active composer has a user-var layer but NO system-instruction layer at all — no `global-directive` / legacy `directive-field` / persona layer in `layers[]` (the WeSeeYou badge-icon-generator pattern is the canonical case: a user-var goes straight into `contents` with no master directive) — F12 still fires with severity **high** (degraded). Rationale: when only a single user-controlled layer exists, no structural separation is possible regardless of API parameter — whatever the user sends IS the entire model input. Emit `apiParameterContext: { absentSystemInstructionLayer: true, userVarApiParameter, separationVerified: false }` to flag this composition shape distinctly from the layer-order violation case so consumers / remediation can route it differently. Recommendation template variant: "The `{promptId}` composer has only a single user-controlled layer with no master directive — add a global system-instruction layer (a persona / master directive) before user content is composed."
